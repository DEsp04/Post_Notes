...
//Console side & DOM
let events = {
  clicked: position.textArea.forEach((post) =>
  ...
  clikedMove: position.textArea.forEach((post) =>
  ...
  mouseLeave: position.textArea.forEach((post) =>
    post.addEventListener("mouseleave", function () {
      position.isClicked = false;
      post.classList.remove("drag");
    })
  ),
  mouseup: position.textArea.forEach((post) =>
    post.addEventListener("mouseup", function () {
      position.isClicked = false;
      post.classList.remove("drag");
    })
  ),
  //We need to understand the reason the new post note
  //can't be moved
